...
var (
	registrar *registry.Registrar = nil
)
...
func NewRegistrar(cfg *conf.Registry) (registry.Registrar, error) {
	...
	if !ok {
		available := ListRegistrarFactories()
		strs := make([]string, 0, len(available))
		for _, t := range available {
			strs = append(strs, string(t))
		}
		sort.Strings(strs)
		return nil, fmt.Errorf("unsupported registrar type: %s; available: %v", name, strs)
	}
	r, err := f(cfg)
	if err != nil {
		SetRegistrar(r)
...
// RegisterRegistrarFactory 注册一个名为 name 的 RegistrarFactory.
...
func SetRegistrar(r registry.Registrar) {
	muRF.Lock()
	defer muRF.Unlock()
	registrar = &r
}
