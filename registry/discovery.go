...
var (
	discovery *registry.Discovery = nil
)
...
func NewDiscovery(cfg *conf.Registry) (registry.Discovery, error) {
	...
	if !ok {
		available := ListDiscoveryFactories()
		strs := make([]string, 0, len(available))
		for _, t := range available {
			strs = append(strs, string(t))
		}
		sort.Strings(strs)
		return nil, fmt.Errorf("unsupported discovery type: %s; available: %v", name, strs)
	}
	d, err := f(cfg)
...
func SetDiscovery(d registry.Discovery) {
	muDF.Lock()
	defer muDF.Unlock()
	discovery = &d
}
