...
func TestRegisterAndNewLogger(t *testing.T) {
	...
	typ := Type("mock")
	_ = RegisterFactory(typ, func(cfg *conf.Logger) (log.Logger, error) {
...
func TestUnregisterAndUnsupported(t *testing.T) {
	typ := Type("temp")
	_ = RegisterFactory(typ, func(cfg *conf.Logger) (log.Logger, error) { return NewStdLogger(), nil })
...
func TestFactoryReturnsNilLogger(t *testing.T) {
	typ := Type("badfactory")
	_ = RegisterFactory(typ, func(cfg *conf.Logger) (log.Logger, error) { return nil, nil })
	defer Unregister(typ)
