...
func Factory(cfg *conf.Logger) (log.Logger, error) {
	if cfg.GetFluentd() == nil {
		err := errors.New("logger zap config is nil")
		return nil, err
	}
	fcfg := cfg.GetFluentd()
	logS, err := NewFluentLogger(fcfg.GetEndpoint(),
		WithTimeout(fcfg.GetTimeout().AsDuration()),
		WithBufferLimit(int(fcfg.GetBufferLimit())),
		WithTagPrefix(fcfg.GetTagPrefix()),
		WithAsync(fcfg.GetAsync()))
	if err != nil {
		return nil, err
	}
	return logS, nil
