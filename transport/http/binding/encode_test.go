...
func TestEncodeURL(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			request:      &binding.HelloRequest{Name: "go", Sub: &binding.Sub{Name: "cnf"}},
			needQuery:    true,
			want:         "http://helloworld.Greeter/helloworld/go/sub?sub.naming=cnf",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/sub/{sub.naming}",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "cnf"}, UpdateMask: &fieldmaskpb.FieldMask{Paths: []string{"name", "sub.naming"}}},
			needQuery:    false,
			want:         "http://helloworld.Greeter/helloworld/sub/cnf?updateMask=name,sub.naming",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "cnf"}},
			needQuery:    false,
			want:         "http://helloworld.Greeter/helloworld/sub/[cnf]",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/[{name}]/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Name: "test", Sub: &binding.Sub{Name: "cnf"}},
			needQuery:    false,
			want:         "http://helloworld.Greeter/helloworld/[test]/sub/[cnf]",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/[{}]/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "cnf"}},
