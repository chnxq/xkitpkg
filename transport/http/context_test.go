...
func TestContextHeader(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{Header: map[string][]string{"name": {"cnf"}}},
		res:    nil,
		w:      responseWriter{},
	}
	h := w.Header()
	if !reflect.DeepEqual(h, http.Header{"name": {"cnf"}}) {
		t.Errorf("expected %v, got %v", http.Header{"name": {"cnf"}}, h)
	}
}
...
func TestContextForm(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{Header: map[string][]string{"name": {"cnf"}}, Method: http.MethodPost},
		res:    nil,
		w:      responseWriter{},
	}
	form := w.Form()
...
func TestContextBindQuery(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{URL: &url.URL{Scheme: "https", Host: "xkit-dev", RawQuery: "page=2"}},
		res:    nil,
		w:      responseWriter{},
	}
...
func TestContextBindForm(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{URL: &url.URL{Scheme: "https", Host: "xkit-dev"}, Form: map[string][]string{"page": {"2"}}},
		res:    nil,
		w:      responseWriter{},
	}
