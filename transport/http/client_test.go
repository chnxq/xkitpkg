...
func TestDefaultErrorDecoder(t *testing.T) {
	...
	if err.(*cnfserrors.Error).Code != int32(500) {
		t.Errorf("expected %v, got %v", 500, err.(*cnfserrors.Error).Code)
	}
	if err.(*cnfserrors.Error).Message != "hi" {
		t.Errorf("expected %v, got %v", "hi", err.(*cnfserrors.Error).Message)
	}
	if err.(*cnfserrors.Error).Reason != "FOO" {
		t.Errorf("expected %v, got %v", "FOO", err.(*cnfserrors.Error).Reason)
	}
}
...
func TestNewClient(t *testing.T) {
	...
	_, err = NewClient(context.Background(), WithDiscovery(&mockDiscovery{}), WithEndpoint("discovery:///xkit"))
	if err != nil {
		t.Error(err)
	}
	...
	_, err = NewClient(context.Background(), WithDiscovery(&mockDiscovery{}), WithEndpoint("https://xkit.dev/"))
	if err == nil {
