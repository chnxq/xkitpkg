...
var (
	_ xTransport.Server     = (*Server)(nil)
	_ xTransport.Endpointer = (*Server)(nil)
)
...
func (s *Server) listenAndEndpoint() error {
	...
	if s.endpoint == nil {
		...
		addr, err := xTransport.AdjustAddress(s.address, s.lis)
		if err != nil {
			return err
		}

		s.endpoint = xTransport.NewRegistryEndpoint(KindGin, addr)
	}
...
func (s *Server) Start(_ context.Context) error {
	...
	LogInfof("[HTTP/GIN] server listening on: %s", s.lis.Addr().String())
...
func (s *Server) Stop(ctx context.Context) error {
	LogInfo("[HTTP/GIN] server stopping...")

	err := s.server.Shutdown(ctx)
	s.err = nil

	LogInfo("[HTTP/GIN] server stopped.")
	return err
}
